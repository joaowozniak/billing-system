...
class AdvanceDto:
    ...
    def __init__(self, id: int, customer_id: int, created: datetime.date, total_advanced: str, 
                    fee: str, mandate_id: int, repayment_start_date: datetime.date, repayment_percentage: int):
    ...
    def pay_amount(self, date: datetime.date, amount: float) -> None:
        ...
        if self.to_be_paid <= amount:
            ...
            metadata = json.dumps({})
            response = Utils.execute_post_request(Constants.BILLING_COMPLETE_ENDPOINT(self.id), metadata, str(date))
            ...
            print(f"Updating advance {self.id} debt...")
